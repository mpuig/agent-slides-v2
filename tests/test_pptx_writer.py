...
def find_layout(
    manifest: dict[str, object], required_slots: Iterable[str]
) -> dict[str, object]:
...
def test_write_pptx_renders_auto_shape_primitives(
    tmp_path: Path, shape_type: str, expected_auto_shape
) -> None:
        ...
        slides=[
            Slide(
                ...
                computed={
                    ...
                    "n-shape": shape_computed(
                        x=48.0, y=96.0, width=624.0, height=180.0
                    ),
                },
            )
        ],
...
def test_write_pptx_renders_pattern_nodes_as_shape_and_text_primitives(
    tmp_path: Path,
) -> None:
    ...
    assert any(
        shape.has_text_frame and shape.text_frame.text == "87%" for shape in shapes
    )
    assert any(
        shape.has_text_frame and shape.text_frame.text == "Adoption" for shape in shapes
    )
...
def test_write_pptx_renders_native_category_charts(
    tmp_path: Path, chart_type: str, pptx_type, series
) -> None:
    ...
    assert [series_item.name for series_item in chart.series] == [
    ...
    if len(chart.series) > 1:
        assert chart.series[1].format.fill.fore_color.rgb == RGBColor.from_string(
            "004E89"
        )


def test_write_pptx_renders_native_scatter_chart_with_read_back_data(
    tmp_path: Path,
) -> None:
                ...
                nodes=[
                    Node(
                        ...
                        chart_spec=ChartSpec(
                            ...
                            style=ChartStyle(
                                has_legend=False, series_colors=["#AA0000", "#0055CC"]
                            ),
                        ),
                    )
                ],
                computed={
                    "n-chart": chart_computed(
                        x=96.0, y=120.0, width=500.0, height=260.0
                    )
                },
            )
        ],
    ...
    assert [
        point.text for point in xml.findall(".//c:xVal//c:numCache/c:pt/c:v", CHART_NS)
    ] == [
        "0.7",
        "1.8",
        "2.6",
    ...
    assert [
        point.text for point in xml.findall(".//c:yVal//c:numCache/c:pt/c:v", CHART_NS)
    ] == [
        "2.7",
        "3.2",
        "0.8",
...
def test_write_pptx_renders_native_table_with_alignment_and_striping(
    tmp_path: Path,
) -> None:
                    ...
                    Node(
                        ...
                        content=NodeContent(
                            blocks=[
                                ...
                                TextBlock(
                                    type="bullet", text="Nested takeaway", level=1
                                ),
                            ]
                        ),
                    )
...
def test_write_pptx_uses_block_positions_for_precise_text_boxes(tmp_path: Path) -> None:
    ...
    assert (
        paragraphs[0].find("./a:pPr/a:spcAft/a:spcPts", DRAWING_NS).attrib["val"]
        == "1000"
    )
    assert any(
        paragraph.find("./a:pPr/a:buChar", DRAWING_NS) is not None
...
def test_write_pptx_renders_icon_nodes_and_icon_bullets(tmp_path: Path) -> None:
    ...
    paragraphs = slide_xml.findall(".//a:p", DRAWING_NS)
    fills = [
        slide.shapes[index].fill.fore_color.rgb for index in range(1, len(slide.shapes))
    ]

    assert len(slide.shapes) >= 3
    assert len(custom_geometry) >= 2
    assert text_frame.paragraphs[0].text == "On track for Q3"
    assert text_frame.paragraphs[0].runs[0].font.color.rgb == RGBColor.from_string(
        "1A73E8"
    )
...
def test_write_pptx_applies_conditional_text_colors_from_design_rules(
    tmp_path: Path,
) -> None:
                    ...
                    Node(
                        ...
                        content=NodeContent(
                            blocks=[
                                TextBlock(
                                    type="paragraph",
                                    text="Revenue +23% is urgent but margin -5%",
                                )
                            ]
                        ),
                    )
    ...
    runs = (
        open_presentation(output_path).slides[0].shapes[0].text_frame.paragraphs[0].runs
    )

    assert any(
        run.text == "+23%" and run.font.color.rgb == RGBColor.from_string("1B8A2D")
        for run in runs
    )
    assert any(
        run.text == "-5%" and run.font.color.rgb == RGBColor.from_string("D32F2F")
        for run in runs
    )
    assert any(
        run.text == "urgent"
        and run.font.bold is True
        and run.font.color.rgb == RGBColor.from_string("C98E48")
        for run in runs
    )


def test_write_pptx_applies_conditional_point_colors_to_single_series_charts(
    tmp_path: Path,
) -> None:
                ...
                nodes=[
                    Node(
                        ...
                        chart_spec=ChartSpec(
                            ...
                            categories=["Q1", "Q2", "Q3"],
                            series=[
                                ChartSeries(name="Revenue", values=[12.0, -4.0, 18.0])
                            ],
...
def test_write_pptx_applies_highlight_and_muted_chart_point_colors(
    tmp_path: Path,
) -> None:
                ...
                nodes=[
                    Node(
                        ...
                        chart_spec=ChartSpec(
                            ...
                            categories=["A", "B", "C"],
                            series=[
                                ChartSeries(name="Share", values=[40.0, 35.0, 25.0])
                            ],
...
def test_write_pptx_clones_template_and_fills_native_placeholders(
    tmp_path: Path,
) -> None:
    ...
    title_placeholder = presentation.slides[0].placeholders[
        title_layout["slot_mapping"]["heading"]
    ]
    subtitle_placeholder = presentation.slides[0].placeholders[
        title_layout["slot_mapping"]["subheading"]
    ]
    body_placeholder = presentation.slides[1].placeholders[
        body_layout["slot_mapping"]["body"]
    ]

    assert [
        paragraph.text for paragraph in subtitle_placeholder.text_frame.paragraphs
    ] == ["Line 1", "Line 2"]
    assert [paragraph.text for paragraph in body_placeholder.text_frame.paragraphs] == [
        "Point A",
        "Point B",
    ]
...
def test_write_pptx_applies_computed_font_styles_to_template_placeholders(
    tmp_path: Path,
) -> None:
            ...
            Slide(
                ...
                nodes=[
                    Node(
                        ...
                        slot_binding="heading",
                        type="text",
                        content="A longer template heading",
                    ),
                    Node(
                        node_id="n-2",
                        slot_binding="body",
...
def test_write_pptx_fills_non_placeholder_text_box_slots_in_template_clone_path(
    tmp_path: Path,
) -> None:
    ...
    quote_shape = next(
        ...
        and shape.text_frame.text == "Stay hungry, stay foolish."
    )
