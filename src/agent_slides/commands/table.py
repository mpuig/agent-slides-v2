...
def _parse_table_json(raw: str, *, option_name: str) -> dict[str, Any]:
    try:
...
def _load_table_data(data_json: str | None, data_file: str | None) -> dict[str, Any]:
    ...
    if data_json is not None:
        return _parse_table_json(data_json, option_name="--data")
    ...
    try:
        ...
        raise AgentSlidesError(FILE_NOT_FOUND, f"Table data file not found: {data_path}") from exc
    except OSError as exc:
        raise AgentSlidesError(
            SCHEMA_ERROR,
            f"Failed to read table data file {data_path}: {exc.strerror or str(exc)}",
        ) from exc

    return _parse_table_json(payload, option_name="--data-file")
...
def add_table_command(
    ...
    """Create or replace a table node in a slide slot."""

    mutation_args: dict[str, object] = {
        "slide": slide_ref,
        "slot": slot_name,
        "data": _load_table_data(data_json, data_file),
    }
